...
import (
	...
	"github.com/magic-lib/go-plat-utils/utils/structx"
...
func (c *Container) Register(bean any, opts ...ComponentOption) {
	...
	if _, ok := c.components[componentName]; ok {
		log.Printf("the component name [%s] has been registered to the container. Repeat the registration...", componentName)
	}

	log.Printf("ioc register : %s = %s.%s", componentName, indirectCType.PkgPath(), indirectCType.Name())
	c.components[componentName] = component
}

// Inject 注册对象实例的字段含有注入标签或者Setter方法，则注入对应组件实例
func (c *Container) Inject(ctx context.Context, obj any) error {
...
// InjectComponents 对所有组件实例执行Inject。即为实例字段注入依赖的组件实例
func (c *Container) InjectComponents() error {
	componentsGroups := utils.ArraySplit[*Component](lo.Values(c.components), 10)

	childCtx, cancel := context.WithCancel(context.Background())
	defer cancel()
	errGroup, _ := errgroup.WithContext(childCtx)

	for _, components := range componentsGroups {
		errGroup.Go(func() error {
			for _, v := range components {
				if err := c.Inject(childCtx, v.Value); err != nil {
					cancel() // 取消所有协程的执行
					return err
				}
			}
...
func (c *Container) GetBeansByType(fieldType reflect.Type) []any {
	return utils.ArrayMap(c.GetComponentsByType(fieldType), func(comp *Component) any { return comp.Value })
}
...
func (c *Container) injectWithField(context context.Context, objValue reflect.Value) error {
	...
	log.Printf("start ioc inject with field: %s.%s", objType.PkgPath(), objType.Name())
...
func (c *Container) injectByName(context context.Context, structType reflect.Type, field reflect.StructField, fieldValue reflect.Value, componentName string) error {
	...
	log.Printf("ioc field inject by name => [%s (%s) -> %s.%s#%s]", componentName, getComponentValueDesc(componentType), structType.PkgPath(), structType.Name(), field.Name)
...
func (c *Container) injectByType(context context.Context, structType reflect.Type, field reflect.StructField, fieldValue reflect.Value) error {
	...
	log.Printf("ioc field inject by type => [%s.%s (%s) -> %s.%s#%s]", fieldType.PkgPath(), fieldType.Name(), getComponentValueDesc(component.GetType()), structType.PkgPath(), structType.Name(), field.Name)
...
func (c *Container) injectWithMethod(context context.Context, objValue reflect.Value) error {
	...
	for i := 0; i < objType.NumMethod(); i++ {
		...
		log.Printf(injectInfo)

		if method.Type.NumIn() != 2 {
			log.Printf("%s error: the method cannot be injected if it does not have one parameter", injectInfo)
			continue
		}
